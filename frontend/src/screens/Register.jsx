...
export const Register = () => {
  ...
  const formSubmissionHandler = async (event) => {
    ...
    if (!enteredNameIsValid && !enteredEmailIsValid && !enteredPasswordIsValid) {
      ...
      try {
        const res = await register({ name: enteredName, email: enteredEmail, password: enteredPassword }).unwrap();
  ...
  return (
    <div className='min-h-screen w-full flex justify-center items-center bg-gradient-to-br from-purple-500 to-indigo-600'>
      ...
      >
        ...
        <div className={`mb-5 flex flex-col ${nameInputClassName}`}>
          <label htmlFor="name">
            {nameInputHasError ? 'Invalid ' : ''}Name
          </label>
          <input
            type="text"
            id='name'
